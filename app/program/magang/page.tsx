    ...
    <div className="bg-slate-50 py-24 sm:py-32 min-h-screen">
      <div className="mx-auto max-w-site px-4 sm:px-6">
        <div className="max-w-3xl mb-16 sm:mb-24 text-center mx-auto">
          <Link href="/program" className="inline-flex items-center text-sm font-semibold text-slate-600 hover:text-slate-900 mb-8 transition-colors bg-white px-4 py-2 rounded-full shadow-sm border border-slate-200">
          ...
          <p className="text-lg leading-relaxed text-slate-600">
            Program pelatihan bahasa dan budaya kerja bagi lulusan SMA/SMK untuk mengikuti program magang di Jepang dengan bimbingan komprehensif.
          </p>
        </div>
            ...
            {magangSteps.map((step, idx) => {
              ...
              return (
                <div key={idx} className={`relative flex flex-col md:flex-row items-start ${isEven ? 'md:flex-row-reverse' : ''} group`}>
                  ...
                  <div className="absolute left-0 md:left-1/2 md:-ml-8 flex h-16 w-16 items-center justify-center rounded-full border-4 border-slate-50 bg-slate-900 text-xl font-bold text-white shadow-md z-10 transition-transform group-hover:scale-110">
                    {step.number}
                  </div>
