    ...
    <div className="bg-slate-50 py-16 sm:py-24 min-h-screen">
      <div className="mx-auto max-w-site px-4 sm:px-6">
        <div className="max-w-3xl mb-16 sm:mb-24 text-center mx-auto">
          <Link href="/program" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-brand-600 mb-6 transition-colors">
          ...
          <p className="text-lg leading-relaxed text-slate-600">
            Persiapan bahasa, keterampilan, dan proses menuju kesempatan bekerja di Jepang.
          </p>
        </div>
            ...
            {tokuteiGinouSteps.map((step, idx) => {
              ...
              return (
                <div key={idx} className={`relative flex flex-col md:flex-row items-start ${isEven ? 'md:flex-row-reverse' : ''} group`}>
                  ...
                  {/* Number Badge */}
                  <div className="absolute left-0 md:left-1/2 md:-ml-7 flex h-14 w-14 items-center justify-center rounded-full border-4 border-white bg-brand-600 text-xl font-bold text-white shadow-sm z-10 transition-transform group-hover:scale-110">
                    {step.number}
                  </div>
                  
                  {/* Content Box */}
                  <div className={`ml-20 md:ml-0 md:w-1/2 ${isEven ? 'md:pl-16' : 'md:pr-16 text-left md:text-right'}`}>
                    <div className="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm transition-all duration-300 hover:shadow-xl hover:border-brand-200 relative overflow-hidden">
                      <div className={`absolute top-0 w-2 h-full bg-brand-600 ${isEven ? 'left-0' : 'right-0'}`}></div>
