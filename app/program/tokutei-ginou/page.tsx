...
export const metadata: Metadata = {
  title: "Program Tokutei Ginou (TG) | LPK Panca Multiguna Sukses Karawang",
    ...
    <div className="bg-slate-50 py-24 sm:py-32 min-h-screen">
      <div className="mx-auto max-w-site px-4 sm:px-6">
        <div className="max-w-3xl mb-16 sm:mb-24 text-center mx-auto">
          <Link href="/program" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-primary-600 mb-8 transition-colors bg-white px-4 py-2 rounded-full shadow-sm border border-slate-200">
            ...
            {tokuteiGinouSteps.map((step, idx) => {
              ...
              return (
                <div key={idx} className={`relative flex flex-col md:flex-row items-start ${isEven ? 'md:flex-row-reverse' : ''} group`}>
                  ...
                  <div className="absolute left-0 md:left-1/2 md:-ml-8 flex h-16 w-16 items-center justify-center rounded-full border-4 border-white bg-primary-600 text-xl font-bold text-white shadow-md z-10 transition-transform group-hover:scale-110 group-hover:bg-primary-700">
                    {step.number}
                  </div>
                  
                  <div className={`ml-24 md:ml-0 md:w-1/2 ${isEven ? 'md:pl-16' : 'md:pr-16 text-left md:text-right'}`}>
                    <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm transition-all duration-300 hover:shadow-xl hover:border-primary-300 relative overflow-hidden">
                      ...
                      <ul className={`space-y-3 text-slate-600 ${isEven ? '' : 'md:inline-block md:text-right'}`}>
                        {step.items.map((item, i) => (
                          <li key={i} className="flex items-start md:items-center gap-3">
                            <span className={`mt-1.5 md:mt-0 h-2 w-2 rounded-full bg-primary-500 shrink-0 ${!isEven ? 'md:hidden' : ''}`}></span>
                            <span className="leading-relaxed text-left text-sm sm:text-base">{item}</span>
                            <span className={`hidden md:block h-2 w-2 rounded-full bg-primary-500 shrink-0 ${isEven ? 'md:hidden' : ''}`}></span>
                          </li>
                        ))}
                      </ul>
                    </div>
        ...
        <div className="mt-24 border-t border-slate-200 pt-16 text-center">
          <a
            ...
            className="inline-flex h-14 items-center justify-center rounded-full bg-primary-600 px-10 text-base font-bold text-white shadow-sm transition-all hover:bg-primary-700 hover:shadow-md hover:-translate-y-1"
          >
